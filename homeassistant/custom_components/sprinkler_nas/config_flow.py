"""Config flow for Sprinkler NAS (direct ESP32 mode)."""
...
class SprinklerNASConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    ) -> SprinklerNASOptionsFlow:
        return SprinklerNASOptionsFlow(config_entry)
...
class SprinklerNASOptionsFlow(config_entries.OptionsFlow):
...
from .const import CONF_DEFAULT_DURATION_MIN, CONF_URL, DEFAULT_DURATION_MIN, DOMAIN

_LOGGER = logging.getLogger(__name__)


class CannotConnect(Exception):
    """Raised when we can't reach the server."""


class SprinklerNASConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Sprinkler NAS (manual or zeroconf discovery)."""

    VERSION = 1

    def __init__(self) -> None:
        self._discovered_url: str | None = None
        self._server_name: str = "Sprinkler NAS"
    ...
    async def async_step_user(
        ...
        if user_input is not None:
            url = user_input[CONF_URL].rstrip("/")
            try:
                server_id = await self._test_and_get_id(url)
            except CannotConnect:
                errors["base"] = "cannot_connect"
    ...
    async def async_step_zeroconf(
        ...
        """Handle service discovered via mDNS."""
        host = discovery_info.host
        port = discovery_info.port
        props = discovery_info.properties or {}

        self._server_id = props.get("id", host)
        self._discovered_url = f"http://{host}:{port}"

        # Use the human-readable part of the service name as the title.
        # bonjour-service sets name like "Sprinkler NAS (hostname)"
        raw_name: str = discovery_info.name or ""
        self._server_name = raw_name.split("._")[0] or "Sprinkler NAS"

        await self.async_set_unique_id(self._server_id)
        self._abort_if_unique_id_configured(
            updates={CONF_URL: self._discovered_url}
        )

        self.context["title_placeholders"] = {"name": self._server_name}
        return await self.async_step_zeroconf_confirm()

    async def async_step_zeroconf_confirm(
        self, user_input: dict[str, Any] | None = None
    ) -> config_entries.FlowResult:
        """Confirm the discovered server."""
        if user_input is not None:
            return self.async_create_entry(
                title=self._server_name,
    ...
    async def _test_and_get_id(self, url: str) -> str:
        """Verify the server is reachable and return a stable unique ID.

        Tries GET /api/health; on success uses SERVER_ID txt record if present,
        or falls back to a slug of the URL.
        """
        session = async_get_clientsession(self.hass)
        try:
            async with asyncio.timeout(10):
        ...
        # Try to get a stable server id from /api/health response
        try:
            data = await resp.json()
            self._server_name = data.get("name", "Sprinkler NAS")
        except Exception:  # noqa: BLE001
            pass

        # Derive a unique id from the URL if we don't have one from discovery
        return self._server_id or url.replace("://", "_").replace(":", "_").replace("/", "")


class SprinklerNASOptionsFlow(config_entries.OptionsFlow):
    """Handle options (e.g. default duration)."""
    ...
    async def async_step_init(
        ...
        current = self.config_entry.options.get(
            CONF_DEFAULT_DURATION_MIN, DEFAULT_DURATION_MIN
        )
        return self.async_show_form(
            step_id="init",
            data_schema=vol.Schema(
                {
                    vol.Required(
                        CONF_DEFAULT_DURATION_MIN, default=current
